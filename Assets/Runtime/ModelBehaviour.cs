...
namespace Servable.Runtime
{
    ...
    public abstract class ModelBehaviour: MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                if (methodInfo.GetCustomAttribute<OnAwakeAttribute>(true) == null)
                    continue;
            ...
            foreach (var methodInfo in methods)
            {
                var onDataAttrs = methodInfo.GetCustomAttributes<OnDataAttribute>(true);
            ...
            foreach (var methodInfo in methods)
            {
                var onCmdAttrs = methodInfo.GetCustomAttributes<OnCommandAttribute>(true);
        ...
        private void OnEnable()
        {
            ...
            foreach (var methodInfo in methods)
            {
                if (methodInfo.GetCustomAttribute<OnEnableAttribute>(true) == null)
                    continue;
                if (methodInfo.GetParameters().Length != 0)
                {
        ...
        private void OnDisable()
        {
            ...
            foreach (var methodInfo in methods)
            {
                if (methodInfo.GetCustomAttribute<OnDisableAttribute>(true) == null)
                    continue;
                if (methodInfo.GetParameters().Length != 0)
                {
        ...
        private void OnDestroy()
        {
            ...
            foreach (var methodInfo in methods)
            {
                if (methodInfo.GetCustomAttribute<OnDestroyAttribute>(true) == null)
                    continue;
                if (methodInfo.GetParameters().Length != 0)
                {
            ...
            foreach (var methodInfo in methods2)
            {
                ...
                foreach (var attr in onCmdAttrs)
                {
                    ...
                    if (parameters.Length == 0)
                    {
                        ...
                        if (handler != null)
                        {
                            try { cmd.RemoveListener(handler); } catch (Exception e) { Debug.LogException(e, this); }
                        }
                        continue;
                    }
                    if (parameters.Length == 1)
